...
class TesseractRecognizer:
    ...
    def _decode_crop(self, crop: np.ndarray) -> RecognitionResult:
        ...
        # upscale so height ≥ 64 px — accuracy degrades sharply below this
        h, w = gray.shape[:2]
        if h < 64:
            factor = 64.0 / h
            gray = cv2.resize(gray, (max(1, int(w * factor)), 64),
                              interpolation=cv2.INTER_CUBIC)

        # add white border — tesseract needs space around glyphs
        ...
        for variant in self._make_variants(gray):
            for psm in psm_list:
                ...
                try:
                    ...
                    # prefer highest-confidence result, not longest
                    if conf > best_conf:
                        best_text, best_conf = text, conf
                except Exception:
                    continue
            if best_conf > 0.5:   # good enough — stop trying variants
                break

        return RecognitionResult(best_text.strip(), best_conf)

    @staticmethod
    def _make_variants(gray: np.ndarray) -> list:
        """Otsu binarized (dark text on white) and its inverse (light text on dark)."""
        import cv2
